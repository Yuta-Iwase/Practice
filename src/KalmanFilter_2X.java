...
public class KalmanFilter_2X {
	public static void main(String[] args) throws Exception{
		...
		for(int t=tmax-2 ; t>=0 ; t--){
			//<☆>
			if(gosa >= 10) gosa/= 7.0;
			xs[t][0] = (x[t][0]*(0.2+(0.8/tmax)*t-Math.random()*0.01)+y[t]*(1.8-(0.8/tmax)*t+Math.random()*0.01))/2.0 + (gosa+Math.random()*10);
			System.out.println((0.2+(0.8/tmax)*t-Math.random()*0.01));
			System.out.println((1.8-(0.8/tmax)*t+Math.random()*0.01));
			//</☆>
		}
